...
API_SCRAPE_NAME: str = "hrl_dmd_bids"
...
def _build_url(
    start_date: str,
    end_date: str,
    base_url: str = "https://api.pjm.com/api/v1/hrl_dmd_bids",
) -> str:
    """Build the PJM API URL for Hourly Demand Bid Data."""
...
def _wait_for_data(url: str) -> requests.Response:
    ...
    if not response.content:
        raise DataNotYetAvailable(
            "PJM Hourly Demand Bids API returned empty response"
        )
...
def _pull(
    ...
    """
        ...
        Update Availability: Daily between 12:00 PM and 02:00 PM EPT
                             (10:00 AM and 12:00 PM MST)
...
def _upsert(
    ...
    primary_key: list = ['datetime_beginning_utc', 'datetime_beginning_ept', 'area'],
):
...
def handle_event(payload: dict) -> None:
    ...
    'notifications_pjm_hrl_dmd_bids'.
