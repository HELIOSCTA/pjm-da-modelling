...
API_SCRAPE_NAME: str = "da_transmission_constraints"
...
def _build_url(
    start_date: str,
    end_date: str,
    base_url: str = "https://api.pjm.com/api/v1/da_transconstraints",
) -> str:
    """Build the PJM API URL for DA Binding Transmission Constraints."""
...
def _wait_for_data(url: str) -> requests.Response:
    ...
    if not response.content:
        raise DataNotYetAvailable(
            "PJM DA Transmission Constraints API returned empty response"
        )
...
def _pull(
    ...
    """
        Day-Ahead Binding Transmission Constraints
        https://dataminer2.pjm.com/feed/da_transconstraints/definition

        Posting Frequency: Daily
        Update Availability: Daily after the DA market clears (~13:30 EPT / 11:30 MST)
...
def handle_event(payload: dict) -> None:
    ...
    'notifications_pjm_da_transmission_constraints'.
