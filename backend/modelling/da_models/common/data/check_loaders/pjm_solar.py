"""Print the coalesced PJM solar loader as a wide table.

Mirrors ``check_loaders/pjm_load.py`` and ``pjm_wind.py``. PJM-native solar
forecast is system-wide (no region) and actuals come from
``net_load_actual.solar_gen_mw`` filtered to RTO, so the output is RTO-only —
one row per Date with Source, As of Date, OnPeak / OffPeak / Flat summaries,
and HE1..HE24.

Source flags whether the row came from the DA-cutoff solar forecast parquet
(preferred where 24-hour coverage exists) or RT actuals (fallback). As of
Date is reconstructed from ``date - 1`` for forecast rows; RT rows carry NaT.

Pre-2019-04-02 dates have no solar_gen_mw (PJM did not publish solar actuals
before then) and no forecast either, so they are absent from the coalesced
series.

Usage::

    python -m backend.modelling.da_models.common.data.check_loaders.pjm_solar
...
def _pjm_solar_wide(coalesced: pd.DataFrame) -> pd.DataFrame:
    """Pivot the coalesced PJM solar frame to wide (RTO-only, no region)."""
    if coalesced.empty:
...
def build_pjm_solar_table(
    cache_dir: Path | None = CACHE_DIR,
    lookback_days: int | None = LOOKBACK_DAYS,
) -> pd.DataFrame:
    """Return the wide RTO PJM solar table, sorted Date desc.
    ...
    """
    coalesced = loader.load_solar_coalesced(cache_dir=cache_dir)
    ...
    return _pjm_solar_wide(coalesced)
...
def _print_forward_horizon(
    ...
    """Print the forward horizon from the latest PJM solar publish.

    PJM's native solar forecast is system-wide (RTO) and typically only
    covers D+1, so this section may show a single forecast_date.
    """
    print_header("Forward horizon (latest publish)")

    with pl.timer("load coalesced PJM solar (latest_only=True)"):
        latest = loader.load_solar_coalesced(cache_dir=cache_dir, latest_only=True)
    ...
    print_section("RTO solar — forward horizon")
    table = _pjm_solar_wide(latest)
...
def run(
    ...
    pl = init_logging(name="check_loaders_pjm_solar", log_dir=LOG_DIR)
    try:
        ...
        with pl.timer("load coalesced PJM solar (lead_days=1)"):
            coalesced = loader.load_solar_coalesced(cache_dir=cache_dir)

        if coalesced.empty:
            pl.warning("Coalesced solar frame is empty; nothing to print.")
            return
        ...
        print_section("RTO solar")

        table = _pjm_solar_wide(coalesced)
        if table.empty:
            pl.warning("No solar data in window.")
            return
        ...
        pl.success("Printed RTO solar section.")
    finally:
        pl.close()
