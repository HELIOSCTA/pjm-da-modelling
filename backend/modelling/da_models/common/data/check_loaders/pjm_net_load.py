"""Print the unified PJM RTO supply-demand bundle as a wide table.

For each Date prints six rows: load, solar, wind, net_load (reported),
net_load (implied = load - solar - wind), and delta (reported - implied).
The delta row exists to surface any identity break; under the unified
loader it should be ~0 by construction.

Source flags ``Forecast`` (DA-cutoff vintage with all 24 HEs present) or
``RT`` (PJM RT actuals filling gaps and pre-backfill history). Identity
holds within each (region, date) row group because the unified loader
makes a single forecast-vs-RT decision for all four components
simultaneously — eliminating the cross-source mixing artifact where
load+net_load could fall back to RT independently of solar+wind.

PJM net-load forecast is RTO only (solar/wind forecasts are system-wide),
so this script is RTO-only — use the Meteologica equivalent
(``meteo_net_load.py``) for sub-zonal coverage.

Consumes ``loader.load_pjm_supply_demand_coalesced`` — the identity-safe
primitive that exposes load + solar + wind + net_load together.

Usage::

    python -m backend.modelling.da_models.common.data.check_loaders.pjm_net_load
    python modelling/da_models/common/data/check_loaders/pjm_net_load.py
"""
...
def _pjm_supply_demand_wide_for_region(
    ...
    for (date_, src), grp in df.groupby(["date", "source"], sort=False):
        ...
        as_of = (date_ - timedelta(days=1)) if src == "forecast" else None
        src_label = {"forecast": "Forecast", "rt": "RT"}.get(src, src)
...
def build_pjm_net_load_table(
    region: str = REGIONS[0],
    cache_dir: Path | None = CACHE_DIR,
    lookback_days: int | None = LOOKBACK_DAYS,
) -> pd.DataFrame:
    """Return the wide PJM supply-demand bundle table for ``region``.

    Six rows per (Date, Region): load, solar, wind, net_load (rep),
    net_load (impl), delta (rep-impl).

    ``lookback_days`` trims the frame to the N most recent dates (inclusive
    of the latest date in the data). ``None`` returns every date.
    ...
    if region != "RTO":
        raise ValueError(
            f"region={region!r} not supported; PJM net_load + solar/wind "
            "forecasts are RTO-only. Use meteo_net_load for sub-zonal."
        )
    coalesced = loader.load_pjm_supply_demand_coalesced(
        cache_dir=cache_dir, region=region
    )
    ...
    return _pjm_supply_demand_wide_for_region(coalesced, region)


def _print_pjm_supply_demand_region_block(
    pl,
    coalesced: pd.DataFrame,
    region: str,
    ...
    table = _pjm_supply_demand_wide_for_region(coalesced, region)
    if table.empty:
...
def _print_forward_horizon(
    ...
    """Print the forward horizon from the latest PJM publish.

    PJM's native net-load forecast typically only covers D+1 (the
    DA-cutoff vintage), so this section may show a single forecast_date
    — that's expected, not a bug. RTO-only by upstream design.
    """
    print_header("Forward horizon (latest publish)")

    with pl.timer("load coalesced PJM supply-demand bundle (latest_only=True)"):
        latest = loader.load_pjm_supply_demand_coalesced(
    ...
    for region in regions:
        ...
        table = _pjm_supply_demand_wide_for_region(latest, region)
...
def run(
    ...
    pl = init_logging(name="check_loaders_pjm_net_load", log_dir=LOG_DIR)
    try:
        ...
        with pl.timer("load coalesced PJM supply-demand bundle (lead_days=1)"):
            coalesced = loader.load_pjm_supply_demand_coalesced(
                cache_dir=cache_dir, region="RTO"
            )

        if coalesced.empty:
            pl.warning("Coalesced PJM supply-demand frame is empty; nothing to print.")
            return
        ...
        for region in regions:
            _print_pjm_supply_demand_region_block(pl, coalesced, region, lookback_days)
