"""Print the coalesced Meteologica load loader as a wide table.

Mirror of ``check_loaders/pjm_load.py`` but for the Meteologica forecast
+ PJM RT-fallback signal (``loader.load_meteologica_load_coalesced``).
...
Usage::
    ...
    python -m backend.modelling.da_models.common.data.check_loaders.meteo_load
    python modelling/da_models/common/data/check_loaders/meteo_load.py
"""
...
def _meteo_load_wide_for_region(
    coalesced: pd.DataFrame,
    region: str,
) -> pd.DataFrame:
    """Pivot the coalesced Meteologica load frame to wide for a single region.
...
def build_meteo_load_table(
    region: str = REGIONS[0],
    cache_dir: Path | None = CACHE_DIR,
    lookback_days: int | None = LOOKBACK_DAYS,
) -> pd.DataFrame:
    """Return the wide Meteologica load table for ``region``, sorted Date desc.
    ...
    """
    coalesced = loader.load_meteologica_load_coalesced(cache_dir=cache_dir)
    ...
    return _meteo_load_wide_for_region(coalesced, region)


def _print_meteo_load_region_block(
    pl,
    coalesced: pd.DataFrame,
    region: str,
    lookback_days: int | None,
) -> None:
    """Print one region's Meteologica load section: header, metadata, RT-fallback warning, table."""
    print_section(f"{region} load")

    table = _meteo_load_wide_for_region(coalesced, region)
    if table.empty:
        pl.warning(f"No load data for region={region}.")
        return
...
def _print_forward_horizon(
    ...
    Uses ``latest_only=True`` to surface every forecast_date the most-
    recent ``as_of_date`` covers — typically D+1 .. D+N for whatever N
    Meteologica publishes for that vintage. Filters to forecast rows
    only (RT rows are noise for this view).
    """
    print_header("Forward horizon (latest publish, all regions)")

    with pl.timer("load coalesced Meteologica load (latest_only=True)"):
        latest = loader.load_meteologica_load_coalesced(
    ...
    for region in regions:
        print_section(f"{region} load — forward horizon")
        table = _meteo_load_wide_for_region(latest, region)
...
def run(
    ...
    pl = init_logging(name="check_loaders_meteo_load", log_dir=LOG_DIR)
    try:
        ...
        with pl.timer("load coalesced Meteologica load (lead_days=1)"):
            coalesced = loader.load_meteologica_load_coalesced(cache_dir=cache_dir)

        if coalesced.empty:
            pl.warning("Coalesced Meteologica load frame is empty; nothing to print.")
            return
        ...
        for region in regions:
            _print_meteo_load_region_block(pl, coalesced, region, lookback_days)
