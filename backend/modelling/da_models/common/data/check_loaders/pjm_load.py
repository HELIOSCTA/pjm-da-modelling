...
(fallback for pre-backfill dates). As of Date is reconstructed from
``date - 1`` for forecast rows; RT rows carry NaT.
...
Usage::
    ...
    python -m backend.modelling.da_models.common.data.check_loaders.pjm_load
    python modelling/da_models/common/data/check_loaders/pjm_load.py
"""
...
def _pjm_load_wide_for_region(
    coalesced: pd.DataFrame,
    region: str,
) -> pd.DataFrame:
    """Pivot the coalesced PJM load frame to wide for a single region.
...
def build_pjm_load_table(
    region: str = REGIONS[0],
    cache_dir: Path | None = CACHE_DIR,
    lookback_days: int | None = LOOKBACK_DAYS,
) -> pd.DataFrame:
    """Return the wide PJM load table for ``region``, sorted Date desc.
    ...
    """
    coalesced = loader.load_load_coalesced(cache_dir=cache_dir)
    ...
    return _pjm_load_wide_for_region(coalesced, region)


def _print_pjm_load_region_block(
    pl,
    coalesced: pd.DataFrame,
    region: str,
    lookback_days: int | None,
) -> None:
    """Print one region's PJM load section: header, metadata, RT-fallback warning, table."""
    print_section(f"{region} load")

    table = _pjm_load_wide_for_region(coalesced, region)
    if table.empty:
...
def _print_forward_horizon(
    ...
    """Print the forward multi-day horizon from the latest PJM load publish.

    Uses ``latest_only=True``. PJM's native load forecast typically only
    covers D+1 (the DA-cutoff vintage), so this section may show a single
    forecast_date — that's expected, not a bug.
    """
    print_header("Forward horizon (latest publish, all regions)")

    with pl.timer("load coalesced PJM load (latest_only=True)"):
        latest = loader.load_load_coalesced(cache_dir=cache_dir, latest_only=True)
    ...
    for region in regions:
        ...
        table = _pjm_load_wide_for_region(latest, region)
...
def run(
    ...
    pl = init_logging(name="check_loaders_pjm_load", log_dir=LOG_DIR)
    try:
        ...
        with pl.timer("load coalesced PJM load (lead_days=1)"):
            coalesced = loader.load_load_coalesced(cache_dir=cache_dir)

        if coalesced.empty:
            pl.warning("Coalesced load frame is empty; nothing to print.")
            return
        ...
        for region in regions:
            _print_pjm_load_region_block(pl, coalesced, region, lookback_days)
