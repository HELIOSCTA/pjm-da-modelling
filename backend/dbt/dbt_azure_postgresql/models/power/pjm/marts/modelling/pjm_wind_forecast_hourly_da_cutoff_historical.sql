...
---------------------------
-- PJM 2-Day Wind Forecast — DA Cutoff, full captured history (bias-safe vintage for backtests)
-- For each as_of_date D, takes the latest publish in the window
-- (D 10:00 EPT - 48h, D 10:00 EPT] per (forecast_date x hour_ending) and emits
-- its full delivery horizon. Mirrors the live wind DA-cutoff mart's
...
-- forecast_execution_date is the chosen publish's actual date and may differ
-- from as_of_date for early hours of D when the chosen publish was issued the
-- prior evening.
...
WITH source_forecasts AS (
    SELECT
        ...
        ,hour_ending
        ,wind_forecast
    FROM {{ ref('source_v1_pjm_wind_forecast_hourly_historical') }}
),
