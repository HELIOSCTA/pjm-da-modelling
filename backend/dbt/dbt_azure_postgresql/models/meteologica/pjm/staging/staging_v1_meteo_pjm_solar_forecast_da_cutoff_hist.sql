...
---------------------------
-- Meteologica PJM Solar Forecast — DA Cutoff, full captured history (bias-safe vintage for backtests)
...
-- and emits its full horizon. Mirrors the live solar DA-cutoff mart's 48h
...
-- Meteologica solar publishes once daily at ~09:46 EPT. The 48h window
...
WITH source_forecasts AS (
    SELECT
        ...
        ,region
        ,solar_forecast
    FROM {{ ref('staging_v1_meteologica_pjm_solar_forecast_hourly') }}
),
