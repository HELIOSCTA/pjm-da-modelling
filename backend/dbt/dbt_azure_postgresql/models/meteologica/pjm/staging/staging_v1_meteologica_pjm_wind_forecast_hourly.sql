...
WITH UNIONED AS (
    ...
    SELECT 'RTO'    AS region, update_id, issue_date, forecast_period_start, forecast_mw FROM {{ ref('src_meteo_pjm_rto_wind') }}
    UNION ALL
    SELECT 'MIDATL' AS region, update_id, issue_date, forecast_period_start, forecast_mw FROM {{ ref('src_meteo_pjm_midatl_wind') }}
    UNION ALL
    SELECT 'SOUTH'  AS region, update_id, issue_date, forecast_period_start, forecast_mw FROM {{ ref('src_meteo_pjm_south_wind') }}
    UNION ALL
    SELECT 'WEST'   AS region, update_id, issue_date, forecast_period_start, forecast_mw FROM {{ ref('src_meteo_pjm_west_wind') }}
...
NORMALIZED AS (
    SELECT
        ...
        ,forecast_mw::NUMERIC AS wind_forecast
    FROM UNIONED
),
