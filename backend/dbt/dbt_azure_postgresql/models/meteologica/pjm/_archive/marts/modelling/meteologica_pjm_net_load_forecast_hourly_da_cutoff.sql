...
---------------------------
-- Meteologica PJM Regional Net-Load Forecast -- DA Cutoff (bias-safe for training)
...
-- INNER JOIN on (forecast_date, hour_ending, region): a missing series drops
-- the row (do not impute zero).
---------------------------

WITH load AS (
    SELECT * FROM {{ ref('meteologica_pjm_load_forecast_hourly_da_cutoff') }}
),

solar AS (
    SELECT * FROM {{ ref('meteologica_pjm_solar_forecast_hourly_da_cutoff') }}
),

wind AS (
    SELECT * FROM {{ ref('meteologica_pjm_wind_forecast_hourly_da_cutoff') }}
)
