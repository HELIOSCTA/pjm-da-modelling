...
# SCRAPE
API_SCRAPE_NAME = "instantaneous_wind_gen_v1_2026_APR_28"
...
"""
Instantaneous Wind Generation
https://dataminer2.pjm.com/feed/instantaneous_wind_gen/definition

    Posting Frequency:    Every 15 seconds (literally "instantaneous")

This is *realized* RTO-level wind generation at 15-second grain. Unlike the
hourly ``wind_generation_by_area`` feed, this one is system-wide only — there
...
publication gap). Only fills the RTO row in pjm_wind_gen_rt_hourly; sub-region
rows (MIDATL/WEST/SOUTH) remain HOURLY-only.

At 15 sec/sample, ~240 rows per hour. The 50,000-row request cap supports
~8.6 days; the 7-day default window stays comfortably under it.

Schema (verified 2026-04-28):
    datetime_beginning_utc    datetime  15-second sample (UTC)
    datetime_beginning_ept    datetime  15-second sample (EPT)
    wind_generation_mw        int       realized RTO wind generation (MW)
"""
...
def _pull(
    ...
    """Pull one window of 5-min realized wind generation by area."""

    url = (
        "https://api.pjm.com/api/v1/instantaneous_wind_gen"
