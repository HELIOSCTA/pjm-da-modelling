...
# SCRAPE
API_SCRAPE_NAME = "da_interface_flows_and_limits"
...
"""
PJM Day-Ahead Interface Flows and Limits (da_interface_flows_and_limits)
https://dataminer2.pjm.com/feed/da_interface_flows_and_limits/definition

    Posting Frequency:    Daily (alongside DA market clear).
    First Available:      2014-01-02 (probed 2026-05-08).
...
Schema (probed 2026-05-08):
    ...
    interface_limit_name     str        e.g. APSOUTH, BCPEP, CENTRAL, EAST
    flow_mw                  numeric    DA scheduled flow (MW)
    limit_mw                 numeric    DA flow limit (MW)

Region coverage:
    The feed returns every interface PJM publishes for a given hour;
    no region filter is applied. ~5 interfaces x 24 hours = ~120
    rows/day, well under the 50k single-page cap, so each daily pull
    is one request.
"""
...
def _pull(
    ...
    url: str = f"https://api.pjm.com/api/v1/da_interface_flows_and_limits?rowCount=50000&startRow=1&datetime_beginning_ept={start_date}%20to%20{end_date}&format=csv&subscription-key=0e3e44aa6bde4d5da1699fda4511235e"
        ...
        while current_date <= end_date:
            ...
            if df.empty:
                ...
                logger.section(
                    ...
                    f"interfaces={df['interface_limit_name'].nunique()}..."
                )
                _upsert(df)
                logger.success(
