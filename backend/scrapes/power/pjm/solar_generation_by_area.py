...
# SCRAPE
API_SCRAPE_NAME = "solar_generation_by_area"
...
https://dataminer2.pjm.com/feed/solar_gen/definition
    ...
    First Available:      2019-04-02
    Retention Time:       Indefinite

NOTE: This is *realized* solar generation, not a vintage-stamped forecast.
...
2020-01-01). Solar forecast vintages still need to be captured by daily
snapshotting of the rolling forecast feed; PJM does not publish a
``solar_frcstd_hist`` archive (404 confirmed).
...
Schema:
    ...
    solar_generation_mw       float     realized solar generation (MW)
...
def _pull(
    ...
    """Pull one window of realized solar generation by area."""

    base_url = "https://api.pjm.com/api/v1/solar_gen"
...
def main(
    ...
    ~347 days per request. For backfill from 2019-04-02, ``relativedelta(days=90)``
...
def _backfill(
    ...
    Feed begins 2019-04-02. Wider delta reduces request count vs the daily
