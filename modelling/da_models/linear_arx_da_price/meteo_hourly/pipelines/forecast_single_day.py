"""Single-day ``meteo_hourly`` linear ARX DA-price forecast -- terminal output.

LEAR-style 24-per-hour ridge ARX on Meteologica regional supply-demand
(load/solar/wind/net-load for RTO + MIDATL/WEST/SOUTH), target in asinh
space, recency-weighted; residual-quantile bands. Print layout mirrors
the like-day ``pjm_rto_hourly`` report and the ``pjm_hourly`` sibling, so
the two variants are directly comparable side-by-side.
...
Usage::
    ...
    python -m da_models.linear_arx_da_price.meteo_hourly.pipelines.forecast_single_day
    python modelling/da_models/linear_arx_da_price/meteo_hourly/pipelines/forecast_single_day.py
"""
...
from da_models.linear_arx_da_price.meteo_hourly.builder import build_panel  # noqa: E402
