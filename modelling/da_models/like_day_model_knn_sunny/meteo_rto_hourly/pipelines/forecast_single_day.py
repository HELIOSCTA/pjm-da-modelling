"""Single-day meteo_rto_hourly Sunny-KNN forecast - terminal output.

Sibling of ``pjm_rto_hourly/pipelines/forecast_single_day.py``: same five
printed sections (FORECAST CONFIGURATION, POOL SUMMARY, LIKE-DAY ANALOGS,
DA LMP LIKE-DAY FORECAST, Quantile Bands), same return shape -- but the
target-date query is sourced from Meteologica's latest published RTO
vintage instead of the lead-1 PJM forecasts, with outages and gas
forward-filled. Useful as a sanity check before the 14-day strip.
...
Usage::
    ...
    python -m da_models.like_day_model_knn_sunny.meteo_rto_hourly.pipelines.forecast_single_day
    python modelling/da_models/like_day_model_knn_sunny/meteo_rto_hourly/pipelines/forecast_single_day.py
"""
...
# the map. See ``forecast_next_14_days.POOL_YEAR_MONTHS`` for the
# motivation and the canonical example.
...
def run(
    ...
    """Run the meteo single-day Sunny forecast.

    Returns the ``run_forecast`` dict augmented with ``df_forecast``,
    ``quantiles_table``, ``metrics`` (when actuals are present),
    ``day_type``, ``n_pool``, ``run_id``, ``run_date``.
    """
    ...
    query_frames = build_horizon_query_rows(
        [resolved_date], cache_dir=configs.CACHE_DIR, spec=spec
    )
    query = query_frames[resolved_date]
