"""Single-day backtest for per_hour - 3-hour window features x per-hour matching.
...
def generate(
    ...
    flt_radius: int = configs.PER_HOUR_SPEC.flt_radius,
    ...
    builder = HTMLDashboardBuilder(
        title=f"Per Hour - {target_date}", theme="dark",
    )
    current_group = None
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=None,
                        help="Output directory (default: per_hour/output).")
...
def main() -> None:
    ...
    args = _parse_args()
    pl = init_logging(name="knn_per_hour", log_dir=_MODELLING_ROOT / "logs")
    try:
        generate(
