"""Terminal printers for the meteo_rto_hourly forecast.

Mirror of ``pjm_rto_hourly/printers.py``. The print layout is
data-source-agnostic — the same headers, the same per-HE strips, the
same gradient buckets — so this is a verbatim copy to keep
meteo_rto_hourly standing on its own without a sibling import.
"""
...
def print_analogs(analogs: pd.DataFrame, target_date: date, hub: str) -> None:
    """Top analog days table (simple aggregated view)."""
    target_dow = DAY_ABBR[target_date.weekday()]
    print("\n" + "=" * 90)
...
def _render_per_hour_metric_row(
    ...
    """Per-HE metric row with mean-per-block summary cells."""
...
def print_quantiles(table: pd.DataFrame) -> None:
    """Quantile bands sub-section ($/MWh)."""
