...
Mirror of ``pjm_rto_hourly/metrics.py``. The metrics themselves are
data-source-agnostic (numpy/pandas only), so this is a verbatim copy
to keep meteo_rto_hourly standing on its own without a sibling import.
"""
...
def crps(y_true: np.ndarray, y_pred_df: pd.DataFrame, quantiles: list[float]) -> float:
    ...
    quantile loss rho_q(y, p) = max(q*(y-p), (q-1)*(y-p))."""
