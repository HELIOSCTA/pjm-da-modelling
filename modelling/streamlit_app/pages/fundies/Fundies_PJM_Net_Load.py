"""Fundies — PJM Net Load: per-as-of-date forecast profiles by forecast date."""
...
from da_models.common.data.loader import load_pjm_net_load_forecast  # noqa: E402
...
st.title("Fundies — PJM Net Load")
st.caption(
    "Per-vintage PJM-native net-load forecast profiles. Pick an as-of date to "
    "lock the forecast snapshot; each covered forecast date gets its own daily "
    "profile."
)


@st.cache_data(show_spinner="Loading PJM net_load forecast parquet...")
def _load() -> pd.DataFrame:
    df = load_pjm_net_load_forecast(cache_dir=configs.CACHE_DIR)
    return df.copy() if df is not None else pd.DataFrame()


with st.sidebar:
    st.header("PJM Net Load")
...
if len(df) == 0:
    st.error(
        "No PJM net_load forecast data — check that "
        "`pjm_net_load_forecast_hourly_da_cutoff*.parquet` exists in cache."
    )
    st.stop()
...
def _overview_fig(snap_df: pd.DataFrame, region_label: str) -> go.Figure:
    ...
    fig.update_layout(
        title=f"PJM Net Load Breakdown — {region_label} (as of {as_of})",
...
for fd in forecast_dates:
    ...
    show_ramp = st.toggle(
        "Show ramps",
        key=f"ramp-pjm-{region}-{as_of}-{fd}",
